      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Kecerdasan bakat pelajar — setiap angka diterbitkan daripada bukti program e-Kesatuan
            yang disahkan.
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
            <StatCard
              icon={Users}
              label="Pelajar Dipantau"
              value={String(totalStudents)}
              sub={`${stats.withEvidenceCount} sudah berprofil`}
            />
            <StatCard
              icon={CheckCircle2}
              label="Bukti Diluluskan"
              value={String(stats.approvedEvidenceCount)}
              sub={`daripada ${stats.programmeCount} program disahkan`}
            ...
            <StatCard
              ...
              sub={`skor keseluruhan ≥ ${HIGH_POTENTIAL_THRESHOLD}`}
              iconCls="bg-blue-50 text-blue-600"
            />
            <StatCard
              icon={Gauge}
              label="Purata Skor Keseluruhan"
              value={stats.avgOverall ? String(stats.avgOverall) : '—'}
              sub="purata 3 kekuatan teratas"
              iconCls="bg-amber-50 text-amber-600"
            />
            <StatCard
              icon={FileCheck2}
              label="Program Disahkan"
              value={String(stats.programmeCount)}
              sub="lulus sepenuhnya dan laporan disahkan"
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-xs text-slate-400">
                    (cincin = purata skor pelajar yang mempunyai bukti)
                  </span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {stats.competencyStats.map((c) => {
                    ...
                    return (
                      <div
                        key={c.code}
                        className="rounded-xl border border-slate-200 p-3 flex items-center gap-3 hover:border-indigo-200 transition-colors"
                      >
                        ...
                        <ProgressRing
                          ...
                          size={48}
                          stroke={5}
                          label={`Purata ${competencyName(c.code)}: ${c.avgScore}`}
                        />
                      </div>
                    );
                  })}
                  ...
                  {(['cemerlang', 'baik', 'berkembang', 'perlu'] as Band[]).map((b) => (
                    <span key={b} className="inline-flex items-center gap-1.5">
                      <span
                        ...
                        style={{ backgroundColor: BAND_META[b].hex }}
                      />
                      {BAND_META[b].label}
                    </span>
                  ))}
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                  <h3 className="font-display text-lg font-bold text-slate-900">
                    Pelajar Mengikut Skor Bakat Keseluruhan
                  </h3>
                        ...
                        >
                          ...
                          <td className="py-3 pr-3">
                            <div className="flex items-center gap-3 min-w-0">
                              <Avatar
                                name={
                                  (row.user as { displayName?: string }).displayName ||
                                  row.user.name
                                }
                              />
                              <div className="min-w-0">
                                ...
                                <p className="text-xs text-slate-500 truncate">
                                  {[row.user.matricNumber, row.user.faculty]
                                    .filter(Boolean)
                                    .join(' · ') || row.user.email}
                                </p>
                              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                ...
                <p className="text-xs text-slate-400 mb-4">
                  Dikira daripada bukti semasa — bukan janaan AI.
                </p>
                <div className="space-y-3">
                  {sorotan.length === 0 && (
                    <p className="text-sm text-slate-500 italic">
                      Tiada sorotan lagi — belum ada bukti.
                    </p>
                  )}
                  {sorotan.map((s) => (
                    <div
                ...
                {donutData.length === 0 ? (
                  ...
                  <>
                    <div
                      ...
                      role="img"
                      aria-label={`Taburan ${totalStudents} pelajar mengikut jalur skor`}
                    >
                      ...
                      {donutData.map((d) => (
                        <div key={d.name} className="flex items-center justify-between text-xs">
                          ...
                          <span className="tabular-nums font-semibold text-slate-900">
                            {d.value} (
                            {totalStudents ? Math.round((d.value / totalStudents) * 100) : 0}%)
                          </span>
                        </div>
                      ))}
                    </div>
