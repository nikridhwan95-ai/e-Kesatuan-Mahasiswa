...
import {
  ...
  updatePresentationSessionStatus,
...
export default function PresentationModule({
    ...
    try {
      ...
      const updatedApps = applications.map((app) =>
        app.id === selectedApp.id
          ? {
              ...app,
              presentationDate: selectedSession.date,
              presentationSessionId,
              presentationRoom,
              status: 'Menunggu Pembentangan' as const,
            }
          : app,
      );
  ...
  const displayApps = applications.filter(
    (app) =>
                    ...
                    {displayApps.map((app) => (
                      <tr key={app.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="p-4 text-sm text-slate-600">
                          {new Date(app.startDate || (app as any).date).toLocaleDateString(
                            'ms-MY',
                            { day: 'numeric', month: 'short', year: 'numeric' },
                          )}
                          {app.startDate &&
                            app.startDate !== app.endDate &&
                            ` - ${new Date(app.endDate).toLocaleDateString('ms-MY', { day: 'numeric', month: 'short', year: 'numeric' })}`}
                        </td>
                        ...
                        <td className="p-4">{getPresentationStatusBadge(app)}</td>
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-2">
                  {displayApps.map((app) => (
                    ...
                    >
                      ...
                      <h4
                        className={`font-semibold text-sm leading-tight ${selectedApp?.id === app.id ? 'text-blue-900' : 'text-slate-900'}`}
                      >
                        {app.title}
                      </h4>
                      {app.presentationDate && (
                        <div className="mt-2 flex items-center gap-2 text-xs text-slate-600 bg-slate-100 p-2 rounded-lg">
                          ...
                          <span>
                            {new Date(app.presentationDate).toLocaleDateString()} (
                            {getSessionName(app.presentationSessionId)}){' '}
                            {app.presentationRoom ? `- Bilik ${app.presentationRoom}` : ''}
                          </span>
                        </div>
                      )}
                    </div>
                ...
                {selectedApp ? (
                            ...
                            {presentationSessionId && (
                              <div>
                                ...
                                >
                                  {Array.from({
                                    length:
                                      sessions.find((s) => s.id === presentationSessionId)
                                        ?.roomCount || 1,
                                  }).map((_, i) => (
                          ...
                          <div>
                            ...
                            <p className="text-sm text-emerald-800 mt-1">
                              ...
                              <span className="font-semibold">
                                {selectedApp.presentationDate} (
                                {getSessionName(selectedApp.presentationSessionId)})
                              </span>
                            ...
                            {sessions.find((s) => s.id === selectedApp.presentationSessionId)
                              ?.link && (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-slate-500 mt-1 italic">
                                  Bajet dimohon: RM {selectedApp.budget.toLocaleString()}
                                </p>
                              </div>
                            </div>
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-slate-400">
                    ...
                    <p className="text-sm mt-1">
                      {isReviewer
                        ? 'Untuk menetapkan jadual atau merekod keputusan semakan.'
                        : 'Untuk melihat butiran jadual semakan.'}
                    </p>
                  </div>
                )}
              </div>
