...
export default function AnalyticsDashboard({ currentUserRole }: AnalyticsDashboardProps) {
  ...
  const { totalApproved, totalVerifiedUsed, usagePercentage, remainingBudget, progressColor } =
    useMemo(() => {
      // Filter by academic session and semester
      const filteredApps = applications.filter(
        (app) =>
          app.academicSession === selectedAcademicSession && app.semester === selectedSemester,
      );
      ...
      const verifiedReports = reports.filter(
        (report) => report.status === 'Disahkan' && filteredAppIds.has(report.applicationId),
      );

      const totalApproved = approvedApps.reduce(
        (sum, app) => sum + (app.approvedAmount || app.budget),
        0,
      );
      const totalUsed = verifiedReports.reduce(
        (sum, report) => sum + (report.verifiedBudgetUsed || 0),
        0,
      );
  ...
  const getStats = () => {
    ...
    const approvedBudget = applications
      .filter((a) => a.status === 'Lulus Sepenuhnya')
      .reduce((sum, a) => sum + (a.approvedAmount || a.budget), 0);
    ...
    switch (currentUserRole) {
      ...
      case 'unit_pelaporan':
        return [
          ...
          {
            title: 'Telah Disemak',
            value: (approved + rejected).toString(),
            icon: CheckCircle,
            color: 'text-emerald-600',
            bg: 'bg-emerald-50',
          },
          {
            title: 'Menunggu Semakan KM',
            value: applications
              .filter((a) => a.status === 'Menunggu Pembentangan')
              .length.toString(),
            icon: Users,
            color: 'text-blue-600',
            bg: 'bg-blue-50',
      ...
      case 'admin':
        return [
          ...
          {
            title: 'Menunggu Kelulusan YDP',
            value: applications
              .filter((a) => a.status === 'Menunggu Kelulusan YDP')
              .length.toString(),
            icon: Clock,
            color: 'text-indigo-600',
            bg: 'bg-indigo-50',
          ...
          {
            title: 'Peruntukan Lulus',
            value: `RM ${approvedBudget.toLocaleString()}`,
            icon: TrendingUp,
            color: 'text-emerald-600',
            bg: 'bg-emerald-50',
          },
        ...
        return [
          {
            title: 'Perlu Diluluskan',
            value: applications
              .filter((a) => a.status === 'Menunggu Kelulusan TNC HEPA')
              .length.toString(),
            icon: Clock,
            color: 'text-amber-600',
            bg: 'bg-amber-50',
  ...
  const getRoleTitle = () => {
    switch (currentUserRole) {
      case 'student':
        return 'Papan Pemuka Pelajar';
      case 'unit_semakan':
        return 'Papan Pemuka KM (Unit Semakan)';
      case 'unit_pembentangan':
        return 'Papan Pemuka KM (Unit Pembentangan)';
      case 'unit_kertas_kerja':
        return 'Papan Pemuka KM (Unit Kertas Kerja)';
      case 'unit_pelaporan':
        return 'Papan Pemuka KM (Unit Pelaporan)';
      case 'admin':
        return 'Papan Pemuka System Admin';
      case 'ydp':
        return 'Papan Pemuka Eksekutif (YDP MPP)';
      case 'tnc_hepa':
        return 'Papan Pemuka Eksekutif (TNC HEPA)';
      default:
        return 'Papan Pemuka Analitik';
    }
  };
    ...
    <div className="space-y-6 sm:space-y-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-900 font-display tracking-tight">
                  Had Kewangan Berasaskan Semester
                </h3>
                <p className="text-sm text-slate-500">
                  Pemantauan peruntukan RM200,000 bagi setiap semester.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-4">
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                ...
                <p className="text-xl sm:text-2xl font-bold text-blue-600">
                  RM {totalApproved.toLocaleString()}
                </p>
                <p className="text-[10px] text-slate-400 mt-1">
                  Jumlah peruntukan yang telah diluluskan oleh TNC HEPA
                </p>
              </div>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">
                ...
                <p className="text-xl sm:text-2xl font-bold text-emerald-600">
                  RM {totalVerifiedUsed.toLocaleString()}
                </p>
                <p className="text-[10px] text-slate-400 mt-1">
                  Jumlah peruntukan yang disahkan menggunakan peruntukan BHEP
                </p>
              </div>
            </div>
            ...
            <div className="flex justify-between items-end mb-2">
              <div>
                <p className="text-sm font-medium text-slate-500">
                  Status Penggunaan Peruntukan Semester
                </p>
              </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        {stats.map((stat, index) => (
          <div
            key={index}
            className="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4 sm:gap-5 hover:shadow-md transition-shadow"
          >
      ...
      {currentUserRole !== 'student' ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 shadow-sm">
            <h3 className="text-lg font-bold text-slate-900 mb-6 font-display tracking-tight">
              Trend Program Mengikut Kategori
            </h3>
            <div className="h-72">
          ...
          <div className="bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 shadow-sm">
            <h3 className="text-lg font-bold text-slate-900 mb-6 font-display tracking-tight">
              Status Penggunaan Bajet Keseluruhan
            </h3>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip
                    formatter={(value) => `RM ${Number(value ?? 0).toLocaleString()}`}
                    contentStyle={{
                      borderRadius: '12px',
                      border: 'none',
                      boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)',
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200">
                  <th className="pb-3 px-2 sm:px-4 font-semibold text-slate-500 text-sm">
                    Nama Program
                  </th>
              ...
              <tbody className="text-sm">
                {applications.slice(0, 5).map((app) => (
                  ...
                  >
                    <td className="py-3 sm:py-4 px-2 sm:px-4 font-medium text-slate-900">
                      {app.title}
                    </td>
