...
const statusMap: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  'Draf': { label: 'Draf', color: 'bg-slate-100 text-slate-700 border-slate-200', icon: <FileText className="w-3 h-3" /> },
  'Menunggu Semakan': { label: 'Menunggu Semakan', color: 'bg-blue-50 text-blue-700 border-blue-200', icon: <Clock className="w-3 h-3" /> },
  'Perlu Pembetulan': { label: 'Perlu Pembetulan', color: 'bg-yellow-100 text-yellow-800 border-yellow-300', icon: <AlertTriangle className="w-3 h-3" /> },
  'Menunggu Pembentangan': { label: 'Menunggu Pembentangan', color: 'bg-purple-50 text-purple-700 border-purple-200', icon: <Calendar className="w-3 h-3" /> },
  'Menunggu Kelulusan YDP': { label: 'Menunggu Kelulusan YDP', color: 'bg-indigo-50 text-indigo-700 border-indigo-200', icon: <CheckCircle className="w-3 h-3" /> },
  'Menunggu Kelulusan TNC HEPA': { label: 'Menunggu Kelulusan TNC HEPA', color: 'bg-indigo-50 text-indigo-700 border-indigo-200', icon: <Clock className="w-3 h-3" /> },
  'Lulus Sepenuhnya': { label: 'Lulus Sepenuhnya', color: 'bg-emerald-50 text-emerald-700 border-emerald-200', icon: <CheckCircle className="w-3 h-3" /> },
  'Ditolak': { label: 'Ditolak', color: 'bg-red-50 text-red-700 border-red-200', icon: <XCircle className="w-3 h-3" /> },
  'Dibatalkan': { label: 'Dibatalkan', color: 'bg-slate-100 text-slate-700 border-slate-200', icon: <XCircle className="w-3 h-3" /> },
};

import { getCurrentAcademicSession, getCurrentSemester, generateAcademicSessions } from '../../utils/dateUtils';

export default function ApplicationModule({ currentUserRole, applicantId }: ApplicationModuleProps) {
  ...
  const handleDeletePaper = async () => {
    ...
    setConfirmModal({
      isOpen: true,
      message: 'Adakah anda pasti mahu memadam kertas kerja lama? Anda perlu memuat naik kertas kerja baharu.',
      onConfirm: async () => {
        try {
  ...
  const handleUpdateStatus = (appId: string, status: ApplicationStatus) => {
    ...
    if (status === 'Perlu Pembetulan' || status === 'Ditolak') {
        const message = status === 'Perlu Pembetulan' ? "Sila nyatakan pembetulan yang diperlukan:" : "Sila nyatakan sebab permohonan ditolak:";
  ...
  const handleSubmitApplication = async (e: React.FormEvent) => {
    ...
    if (!isDraft && isStudent) {
      ...
      if (!isProfileComplete) {
        showNotification('Sila lengkapkan profil anda (No. Matrik, No. Telefon, Fakulti, dan Kolej) sebelum menghantar permohonan.', 'error');
        setLoading(false);
        return;
      }
    }
    ...
    try {
      ...
      if (editingApp) {
        ...
        showNotification(`Permohonan berjaya ${isDraft ? 'disimpan sebagai draf' : 'dihantar'}!`, 'success');
      }
  ...
  const overlays = (
    <>
      {notification && (
        <div className={`fixed top-4 right-4 z-50 px-6 py-3 rounded-xl shadow-lg flex items-center gap-3 transition-all transform duration-300 ${
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="p-4 sm:p-8 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className="text-[10px] sm:text-sm font-bold text-slate-400 tracking-wider uppercase">{selectedApp.id}</span>
                {currentStatus && (
                  <span className={`px-2.5 py-1 rounded-full text-[10px] sm:text-xs font-semibold flex items-center gap-1.5 ${currentStatus.color}`}>
                    {currentStatus.icon} {currentStatus.label}
                  </span>
                )}
              </div>
              <h2 className="text-xl sm:text-2xl font-bold text-slate-900 font-display">{selectedApp.title}</h2>
            </div>
            {isStudent && (
          ...
          <div className="p-4 sm:p-8 grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
            <div className="md:col-span-2 space-y-6 sm:space-y-8">
              <section>
                <h3 className="text-xs sm:text-sm font-bold text-slate-900 uppercase tracking-wider mb-4 border-b border-slate-100 pb-2">Maklumat Program</h3>
                
                {(selectedApp.status === 'Perlu Pembetulan' || selectedApp.status === 'Ditolak') && (
                  <div className={`mb-6 ${selectedApp.status === 'Ditolak' ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200'} border rounded-xl p-4 animate-in fade-in slide-in-from-top-2 duration-300`}>
                    <h4 className={`text-sm font-bold ${selectedApp.status === 'Ditolak' ? 'text-red-900' : 'text-amber-900'} mb-1 flex items-center gap-2`}>
                      {selectedApp.status === 'Ditolak' ? <XCircle className="w-4 h-4" /> : <AlertTriangle className="w-4 h-4" />}
                      Catatan Kesatuan Mahasiswa:
                    </h4>
                    <p className={`text-sm ${selectedApp.status === 'Ditolak' ? 'text-red-800' : 'text-amber-800'}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                  <div>
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Jawatan Pemohon</p>
                    <p className="text-slate-900 font-medium">{selectedApp.applicantPosition || '-'}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Peringkat Penganjuran</p>
                    <p className="text-slate-900 font-medium">{selectedApp.organizingLevel || '-'}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Tarikh Program</p>
                    <p className="text-slate-900 font-medium flex items-center gap-1.5">
                      <Calendar className="w-4 h-4 text-slate-400" /> 
                      {new Date(selectedApp.startDate || (selectedApp as any).date).toLocaleDateString('ms-MY', { day: 'numeric', month: 'short', year: 'numeric' })} 
                      {selectedApp.startDate && selectedApp.startDate !== selectedApp.endDate && ` - ${new Date(selectedApp.endDate).toLocaleDateString('ms-MY', { day: 'numeric', month: 'short', year: 'numeric' })}`}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Tempat</p>
                    <p className="text-slate-900 font-medium">{selectedApp.venue || '-'}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Penceramah</p>
                    <p className="text-slate-900 font-medium">{selectedApp.speaker || '-'}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Bajet Dimohon</p>
                    <p className="text-slate-900 font-medium flex items-center gap-1.5">RM {selectedApp.budget.toLocaleString()}</p>
                  </div>
                  ...
                  {!isStudent && (
                    <div>
                      <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Pemohon</p>
                      <p className="text-slate-900 font-medium">{usersMap[selectedApp.applicantId] || 'Tiada Rekod'}</p>
                    </div>
                  )}
                </div>
              </section>
              ...
              <section>
                <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4 border-b border-slate-100 pb-2">Objektif Utama</h3>
                <p className="text-slate-700 leading-relaxed">{selectedApp.objective}</p>
              </section>

              <section>
                <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4 border-b border-slate-100 pb-2">Impak Program (Kemahiran Insaniah)</h3>
              ...
              <section>
                <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4 border-b border-slate-100 pb-2">Dokumen Sokongan</h3>
                    ...
                    >
                      ...
                      {correctionFile ? (
                          <div className="animate-in fade-in zoom-in duration-300">
                              ...
                              <p className="text-xs text-emerald-500 mt-1">{(correctionFile.size / 1024 / 1024).toFixed(2)} MB</p>
                              <p className="text-xs text-emerald-600 mt-2 font-medium">Klik untuk tukar fail</p>
                          </div>
                      ) : (
                          <>
                              <Upload className="w-10 h-10 text-slate-400 mx-auto mb-3 group-hover:text-blue-500 transition-colors" />
                              <p className="text-sm text-slate-600 font-medium">Muat Naik Kertas Kerja Baharu</p>
            ...
            <div className="space-y-6">
              <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200">
                <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4">Jejak Status</h3>
                <div className="space-y-6 relative before:absolute before:inset-0 before:ml-3 before:-translate-x-px before:h-full before:w-0.5 before:bg-slate-200">
                  ...
                  {['Menunggu Pembentangan', 'Menunggu Kelulusan YDP', 'Menunggu Kelulusan TNC HEPA', 'Lulus Sepenuhnya'].includes(selectedApp.status) && (
                    <div className="relative flex items-start gap-4">
                      <div className={`flex items-center justify-center w-6 h-6 rounded-full border-2 border-white ${['Menunggu Kelulusan YDP', 'Menunggu Kelulusan TNC HEPA', 'Lulus Sepenuhnya'].includes(selectedApp.status) ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-400'} shadow shrink-0 z-10 mt-1`}>
                        {['Menunggu Kelulusan YDP', 'Menunggu Kelulusan TNC HEPA', 'Lulus Sepenuhnya'].includes(selectedApp.status) ? <CheckCircle className="w-3 h-3" /> : <Clock className="w-3 h-3" />}
                      </div>
                      <div className="flex-1 p-4 rounded-xl border border-slate-200 bg-white shadow-sm">
                        <div className="flex items-center justify-between space-x-2 mb-1">
                          <div className="font-bold text-slate-900 text-sm">Sesi Semakan KM</div>
                          {['Menunggu Kelulusan YDP', 'Menunggu Kelulusan TNC HEPA', 'Lulus Sepenuhnya'].includes(selectedApp.status) && <time className="font-mono text-xs text-slate-500">14 Okt</time>}
                        </div>
                        <div className="text-slate-500 text-xs mb-3">
                          {['Menunggu Kelulusan YDP', 'Menunggu Kelulusan TNC HEPA', 'Lulus Sepenuhnya'].includes(selectedApp.status) ? 'Semakan selesai dan disokong oleh KM.' : 'Menunggu sesi semakan bersama KM.'}
                        </div>
                        ...
                        {selectedApp.status === 'Menunggu Pembentangan' && (
                          <div className="mt-4 pt-4 border-t border-slate-100">
                            <div className="flex items-center gap-2 mb-3">
                              ...
                              <p className="text-xs font-bold text-slate-900 uppercase tracking-wider">Jadual Semakan Rasmi</p>
                            </div>
                            ...
                            {selectedApp.presentationDate ? (
                              <div className="bg-slate-50 rounded-xl border border-slate-200 overflow-hidden">
                                <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                  <div className="space-y-3">
                                    <div className="flex items-start gap-3">
                                      ...
                                      <div>
                                        <p className="text-[10px] uppercase font-bold text-slate-400 leading-none mb-1">Tarikh</p>
                                        <p className="text-sm font-semibold text-slate-900">
                                          {new Date(selectedApp.presentationDate).toLocaleDateString('ms-MY', { day: 'numeric', month: 'long', year: 'numeric' })}
                                        </p>
                                      </div>
                                    </div>
                                    ...
                                    <div className="flex items-start gap-3">
                                      ...
                                      <div>
                                        <p className="text-[10px] uppercase font-bold text-slate-400 leading-none mb-1">Masa</p>
                                        <p className="text-sm font-semibold text-slate-900">
                                          {sessions.find(s => s.id === selectedApp.presentationSessionId)?.time || 'Akan dimaklumkan'}
                                        </p>
                                      </div>
                                    </div>
                                  </div>
                                  ...
                                  <div className="space-y-3">
                                    <div className="flex items-start gap-3">
                                      ...
                                      <div>
                                        <p className="text-[10px] uppercase font-bold text-slate-400 leading-none mb-1">Bilik / Lokasi</p>
                                        <p className="text-sm font-semibold text-slate-900">
                                          {selectedApp.presentationRoom ? `Bilik ${selectedApp.presentationRoom}` : 'Akan dimaklumkan'}
                                        </p>
                                      </div>
                                    </div>
                                    ...
                                    {sessions.find(s => s.id === selectedApp.presentationSessionId)?.link && (
                                      <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                          <p className="text-[10px] uppercase font-bold text-slate-400 leading-none mb-1">Pautan Maya</p>
                                          <a 
                                            href={sessions.find(s => s.id === selectedApp.presentationSessionId)?.link} 
                                            target="_blank" 
                                            rel="noreferrer" 
                                ...
                                <div className="bg-blue-600 p-2 text-center">
                                  <p className="text-[10px] text-white font-medium uppercase tracking-widest">Sila hadir 10 minit lebih awal</p>
                                </div>
                              </div>
                            ) : (
                              <div className="flex items-center gap-3 p-4 rounded-xl bg-amber-50 border border-amber-100 text-amber-800">
                                <Clock className="w-5 h-5 opacity-50" />
                                <p className="text-xs font-medium">Jadual semakan sedang dikemas kini oleh Unit Pembentangan. Sila semak semula sebentar lagi.</p>
                              </div>
                            )}
                          </div>
                        )}
                  ...
                  {['Menunggu Kelulusan YDP', 'Menunggu Kelulusan TNC HEPA', 'Lulus Sepenuhnya'].includes(selectedApp.status) && (
                    <div className="relative flex items-start gap-4">
                      <div className={`flex items-center justify-center w-6 h-6 rounded-full border-2 border-white ${['Menunggu Kelulusan TNC HEPA', 'Lulus Sepenuhnya'].includes(selectedApp.status) ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-400'} shadow shrink-0 z-10 mt-1`}>
                        {['Menunggu Kelulusan TNC HEPA', 'Lulus Sepenuhnya'].includes(selectedApp.status) ? <CheckCircle className="w-3 h-3" /> : <Clock className="w-3 h-3" />}
                      </div>
                      <div className={`flex-1 p-4 rounded-xl border ${['Menunggu Kelulusan TNC HEPA', 'Lulus Sepenuhnya'].includes(selectedApp.status) ? 'border-blue-100 bg-blue-50' : 'border-slate-200 bg-white'} shadow-sm`}>
                        <div className="flex items-center justify-between space-x-2 mb-1">
                          <div className={`font-bold text-sm ${['Menunggu Kelulusan TNC HEPA', 'Lulus Sepenuhnya'].includes(selectedApp.status) ? 'text-blue-900' : 'text-slate-900'}`}>Kelulusan YDP MPP</div>
                          {['Menunggu Kelulusan TNC HEPA', 'Lulus Sepenuhnya'].includes(selectedApp.status) && <time className="font-mono text-xs text-blue-600">15 Okt</time>}
                        </div>
                        <div className={`text-xs ${['Menunggu Kelulusan TNC HEPA', 'Lulus Sepenuhnya'].includes(selectedApp.status) ? 'text-blue-700' : 'text-slate-500'}`}>
                          {['Menunggu Kelulusan TNC HEPA', 'Lulus Sepenuhnya'].includes(selectedApp.status) ? 'Disokong oleh YDP MPP.' : 'Menunggu sokongan YDP MPP.'}
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {['Menunggu Kelulusan TNC HEPA', 'Lulus Sepenuhnya'].includes(selectedApp.status) && (
                    <div className="relative flex items-start gap-4">
                      <div className={`flex items-center justify-center w-6 h-6 rounded-full border-2 border-white ${selectedApp.status === 'Lulus Sepenuhnya' ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-400'} shadow shrink-0 z-10 mt-1`}>
                        {selectedApp.status === 'Lulus Sepenuhnya' ? <CheckCircle className="w-3 h-3" /> : <Clock className="w-3 h-3" />}
                      </div>
                      <div className={`flex-1 p-4 rounded-xl border ${selectedApp.status === 'Lulus Sepenuhnya' ? 'border-emerald-100 bg-emerald-50' : 'border-slate-200 bg-white'} shadow-sm`}>
                        <div className="flex items-center justify-between space-x-2 mb-1">
                          <div className={`font-bold text-sm ${selectedApp.status === 'Lulus Sepenuhnya' ? 'text-emerald-900' : 'text-slate-900'}`}>Kelulusan Akhir (TNC HEPA)</div>
                          {selectedApp.status === 'Lulus Sepenuhnya' && <time className="font-mono text-xs text-emerald-600">16 Okt</time>}
                        </div>
                        <div className={`text-xs ${selectedApp.status === 'Lulus Sepenuhnya' ? 'text-emerald-700' : 'text-slate-500'}`}>
                          {selectedApp.status === 'Lulus Sepenuhnya' ? 'Permohonan diluluskan sepenuhnya.' : 'Menunggu kelulusan TNC HEPA.'}
                        </div>
                      </div>
                    </div>
                  )}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1.5">
            {isStudent ? 'Urus dan pantau status kertas kerja program anda.' : 'Pantau dan urus semua permohonan program pelajar.'}
          </p>
        </div>
        ...
        {isStudent && (
            ...
            onClick={() => {
              if (isFormOpen) {
                ...
                if (isStudent) {
                  ...
                  if (!isProfileComplete) {
                    showNotification('Sila lengkapkan profil anda (No. Matrik, No. Telefon, Fakulti, dan Kolej) sebelum membuat permohonan.', 'error');
                    return;
                  }
                }
                setIsFormOpen(true);
          ...
          >
            {isFormOpen ? 'Batal' : <><Plus className="w-5 h-5" /> Mohon Baru</>}
          </button>
        )}
      </div>
          ...
          {openSessions.length === 0 && (
            <div className="mx-8 mt-8 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-amber-700 mt-1">Buat masa sekarang, tiada sesi semakan yang dibuka oleh Kesatuan Mahasiswa. Anda masih boleh menghantar permohonan, tetapi ia akan diletakkan dalam senarai menunggu sehingga sesi baharu dibuka.</p>
              </div>
            </div>
          )}
          ...
          <form className="p-4 sm:p-8 space-y-4 sm:space-y-6" onSubmit={handleSubmitApplication}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="col-span-1 md:col-span-2">
                <label className="block text-sm font-semibold text-slate-700 mb-2">Jawatan Pemohon (Deklarasi)</label>
                <select name="applicantPosition" defaultValue={editingApp?.applicantPosition} className="w-full border border-slate-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow" required>
                  <option value="">Pilih Jawatan...</option>
                  <option value="Pengarah">Pengarah Program</option>
                  <option value="Setiausaha">Setiausaha Program</option>
                </select>
                <p className="text-xs text-slate-500 mt-1 italic">*Hanya Pengarah atau Setiausaha Program sahaja yang dibenarkan untuk menghantar permohonan.</p>
              </div>
              <div className="col-span-1 md:col-span-2">
                <label className="block text-sm font-semibold text-slate-700 mb-2">Tajuk Program</label>
                <input type="text" name="title" defaultValue={editingApp?.title} className="w-full border border-slate-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow" placeholder="Contoh: Karnival Sukan..." required />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Sesi Akademik</label>
                <select name="academicSession" defaultValue={editingApp?.academicSession || currentAcademicSession} className="w-full border border-slate-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow disabled:bg-slate-100 disabled:text-slate-500" required disabled={isAmendment}>
              ...
              <div>
                ...
                <select name="semester" defaultValue={editingApp?.semester || currentSemester} className="w-full border border-slate-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow disabled:bg-slate-100 disabled:text-slate-500" required disabled={isAmendment}>
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Tempat Program</label>
                <input type="text" name="venue" defaultValue={editingApp?.venue} className="w-full border border-slate-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow" placeholder="Contoh: Dewan Besar UPM" required />
              </div>
              <div className="col-span-1 md:col-span-2">
                <label className="block text-sm font-semibold text-slate-700 mb-2">Penceramah / Tetamu Jemputan</label>
                <input type="text" name="speaker" defaultValue={editingApp?.speaker} className="w-full border border-slate-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow" placeholder="Contoh: Dr. Ahmad (Jika ada)" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Bajet Dimohon (RM)</label>
                <input 
                  type="number" 
                  name="budget" 
              ...
              <div className="col-span-1 md:col-span-2">
                <label className="block text-sm font-semibold text-slate-700 mb-2">Peringkat Penganjuran</label>
                <select name="organizingLevel" defaultValue={editingApp?.organizingLevel} className="w-full border border-slate-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow disabled:bg-slate-100 disabled:text-slate-500" required disabled={isAmendment}>
              ...
              <div className="col-span-1 md:col-span-2">
                ...
                <select name="category" defaultValue={editingApp?.category?.toLowerCase()} className="w-full border border-slate-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow disabled:bg-slate-100 disabled:text-slate-500" required disabled={isAmendment}>
              ...
              <div className="col-span-1 md:col-span-2">
                <label className="block text-sm font-semibold text-slate-700 mb-2">Sesi Semakan (Pilihan)</label>
                <select 
                  className="w-full border border-slate-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow disabled:bg-slate-100 disabled:text-slate-500"
                  value={newAppSession}
                ...
                >
                  <option value="">{openSessions.length === 0 ? 'Tiada Sesi Dibuka' : 'Pilih Sesi Semakan...'}</option>
                  {openSessions.map(session => (
                    <option key={session.id} value={session.id}>{session.name} ({session.date} {session.time})</option>
                  ))}
                </select>
                <p className="text-xs text-slate-500 mt-1">Sila pilih sesi semakan yang dibuka oleh Kesatuan Mahasiswa.</p>
              </div>
            </div>
            
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2">Impak Program (Kemahiran Insaniah)</label>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 bg-slate-50 p-4 rounded-xl border border-slate-200">
                {[
                  ...
                  <label key={skill} className="flex items-center gap-3 cursor-pointer group">
                    ...
                    />
                    <span className="text-sm text-slate-600 group-hover:text-slate-900 transition-colors">{skill}</span>
                  </label>
                ))}
              </div>
              <p className="text-xs text-slate-500 mt-2 italic">*Sila tandakan kemahiran insaniah yang berkaitan dengan program ini.</p>
            </div>

            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2">Objektif Utama</label>
              <textarea name="objective" defaultValue={editingApp?.objective} className="w-full border border-slate-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow disabled:bg-slate-100 disabled:text-slate-500" rows={3} placeholder="Nyatakan objektif program..." required disabled={isAmendment}></textarea>
            </div>

            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2">Muat Naik Kertas Kerja (PDF)</label>
              <div 
              ...
              >
                ...
                {paperFile || editingApp?.paperUrl ? (
                  <>
                    <CheckCircle className="w-10 h-10 text-emerald-500 mx-auto mb-3" />
                    <p className="text-sm text-emerald-700 font-medium">{paperFile ? paperFile.name : 'Fail sedia ada'}</p>
                    {paperFile && <p className="text-xs text-emerald-500 mt-1">{(paperFile.size / 1024 / 1024).toFixed(2)} MB</p>}
                    {!paperFile && editingApp?.paperUrl && <p className="text-xs text-emerald-500 mt-1">Klik untuk tukar fail</p>}
                  </>
                ) : (
                  <>
                    <Upload className="w-10 h-10 text-slate-400 mx-auto mb-3 group-hover:text-blue-500 transition-colors" />
        ...
        <div className="block lg:hidden divide-y divide-slate-100">
          {displayedApps.map((app) => (
            <div key={app.id} className="p-4 space-y-4 hover:bg-slate-50 transition-colors">
              <div className="flex justify-between items-start gap-3" onClick={() => setSelectedApp(app)}>
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-slate-900 truncate">{app.title}</h4>
                  <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wider mt-0.5">ID: {app.id}</p>
                </div>
                <span className={`shrink-0 px-2.5 py-1 rounded-full text-[10px] font-bold border uppercase tracking-wider ${
                  app.status === 'Lulus Sepenuhnya' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                  app.status === 'Menunggu Semakan' ? 'bg-blue-50 text-blue-700 border-blue-200' :
              ...
              <div className="grid grid-cols-2 gap-4 text-xs" onClick={() => setSelectedApp(app)}>
                <div className="flex items-center gap-2 text-slate-500">
                  <Calendar className="w-4 h-4 text-slate-400" />
                  <span className="font-medium">{new Date(app.startDate || (app as any).date).toLocaleDateString('ms-MY', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                </div>
              ...
              {!isStudent && (
                <div className="flex items-center gap-2 pt-2 border-t border-slate-100" onClick={() => setSelectedApp(app)}>
                  <div className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-[10px] font-bold text-slate-600">
                    {(usersMap[app.applicantId] || 'T').charAt(0)}
                  </div>
                  <span className="text-xs font-medium text-slate-600 truncate">{usersMap[app.applicantId] || 'Tiada Rekod'}</span>
                </div>
              )}
              ...
              {displayedApps.map((app) => (
                <tr key={app.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4">
                    ...
                    <div className="text-xs text-slate-400 mt-0.5" title={app.id}>ID: {app.id}</div>
                  </td>
                  {!isStudent && (
                    <td className="px-6 py-4">
                      <div className="font-medium text-slate-700">{usersMap[app.applicantId] || 'Tiada Rekod'}</div>
                    </td>
                  )}
                  <td className="px-6 py-4">
