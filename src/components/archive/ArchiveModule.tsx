  ...
  return (
    <div className="space-y-6">
      ...
      {!showApprovalLetter && (
        <>
          <div>
            <h2 className="text-3xl font-bold text-slate-900 font-display tracking-tight">Kertas Kerja yang Diluluskan</h2>
            <p className="text-sm text-slate-500 mt-1.5">Senarai lengkap permohonan kertas kerja yang telah mendapat kelulusan sepenuhnya.</p>
          </div>
      ...
      {!showApprovalLetter && (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left text-slate-600">
            <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-100">
              <tr>
                <th scope="col" className="px-6 py-4 font-semibold">ID Program</th>
                <th scope="col" className="px-6 py-4 font-semibold">Nama Program</th>
                <th scope="col" className="px-6 py-4 font-semibold">Pemohon</th>
                <th scope="col" className="px-6 py-4 font-semibold">Tarikh Program</th>
                <th scope="col" className="px-6 py-4 font-semibold">Sesi</th>
                <th scope="col" className="px-6 py-4 font-semibold text-center">Status Akhir</th>
                <th scope="col" className="px-6 py-4 font-semibold text-right">Tindakan</th>
              </tr>
            </thead>
              ...
              {filteredPrograms.length > 0 ? (
                filteredPrograms.map((app) => (
                  <tr key={app.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 font-medium text-slate-900 whitespace-nowrap">{app.id}</td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border ${
                        ...
                        app.status === 'Dibatalkan' ? 'bg-slate-100 text-slate-600 border-slate-200' :
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/50">
          <span className="text-xs text-slate-500">Menunjukkan <span className="font-semibold text-slate-900">1-{filteredPrograms.length}</span> daripada <span className="font-semibold text-slate-900">{filteredPrograms.length}</span> rekod</span>
          <div className="flex gap-2">
            <button disabled className="px-3 py-1 text-xs font-medium text-slate-400 bg-white border border-slate-200 rounded-lg cursor-not-allowed">Sebelumnya</button>
            <button disabled className="px-3 py-1 text-xs font-medium text-slate-400 bg-white border border-slate-200 rounded-lg cursor-not-allowed">Seterusnya</button>
          </div>
        </div>
      </div>
      )}
