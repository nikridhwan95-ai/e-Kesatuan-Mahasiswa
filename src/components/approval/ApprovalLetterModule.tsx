...
export default function ApprovalLetterModule({ applicationId, onBack }: ApprovalLetterModuleProps) {
  ...
  const letterData = {
    ...
    programDate: application.startDate ? `${new Date(application.startDate).toLocaleDateString('ms-MY', { day: 'numeric', month: 'long', year: 'numeric' })}${application.startDate !== application.endDate ? ` - ${new Date(application.endDate).toLocaleDateString('ms-MY', { day: 'numeric', month: 'long', year: 'numeric' })}` : ''}` : ((application as any).date ? new Date((application as any).date).toLocaleDateString('ms-MY', { day: 'numeric', month: 'long', year: 'numeric' }) : '-'),
    ...
    <div className="space-y-4 sm:space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 print:hidden bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-slate-900 font-display tracking-tight leading-none">Surat Kelulusan</h2>
      ...
      <div className="bg-white p-6 sm:p-12 rounded-xl sm:rounded-none shadow-sm border border-slate-200 max-w-[210mm] mx-auto min-h-[297mm] relative print:shadow-none print:border-none print:p-0 overflow-x-auto">
        ...
        {settings.letterheadUrl ? (
          ...
          <div className="border-b-2 border-slate-900 pb-6 mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-900 uppercase tracking-wide">{settings.organizationName}</h1>
                <p className="text-sm text-slate-600 font-medium">Universiti Putra Malaysia</p>
                <p className="text-xs text-slate-500 mt-1">43400 UPM Serdang, Selangor Darul Ehsan</p>
              </div>
            </div>
        ...
        <div className="space-y-6 text-slate-900 text-justify leading-relaxed font-serif">
          <div className="flex justify-between items-start">
            <div>
              <p className="font-bold">Ruj. Kami: <span className="font-normal">{letterData.refNumber}</span></p>
              <p className="font-bold">Tarikh: <span className="font-normal">{letterData.date}</span></p>
            </div>
          </div>
        ...
        <div className="absolute bottom-12 left-12 right-12 border-t border-slate-200 pt-4 text-center text-[10px] text-slate-400 font-sans print:bottom-8">
          ...
          <p>ID Cetakan: {application.id} • Tarikh Cetakan: {new Date().toLocaleString('ms-MY')}</p>
        </div>
        </div>
      </div>
    </div>
