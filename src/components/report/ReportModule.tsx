...
export default function ReportModule({ currentUserRole, applicantId }: ReportModuleProps) {
  ...
  useEffect(() => {
    if (selectedItem?.report) {
      setVerifiedBudget(selectedItem.report.verifiedBudgetUsed?.toString() || selectedItem.report.unionBudgetUsed?.toString() || '');
    } else {
      setVerifiedBudget('');
    }
  }, [selectedAppId]);
    ...
    <div className="space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="block lg:hidden divide-y divide-slate-100">
            {displayedReports.map((item) => (
              <div key={item.app.id} className="p-4 space-y-4 hover:bg-slate-50 transition-colors" onClick={() => setSelectedAppId(item.app.id)}>
                <div className="flex justify-between items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-slate-900 truncate">{item.app.title}</h4>
                    <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wider mt-0.5">ID Laporan: {item.report?.id || '-'}</p>
                  </div>
                  <span className={`shrink-0 px-2.5 py-1 rounded-full text-[10px] font-bold border uppercase tracking-wider ${
                    item.status === 'Tertunggak' ? 'bg-red-50 text-red-700 border-red-200' : 
                    item.status === 'Dihantar' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                ...
                <div className="grid grid-cols-2 gap-4 text-xs">
                  <div className="flex items-center gap-2 text-slate-500">
                    ...
                    <span className="font-medium">{new Date(item.app.startDate || (item.app as any).date).toLocaleDateString('ms-MY', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                  </div>
                  <div className="flex items-center gap-2 text-slate-600">
                    <User className="w-4 h-4 text-slate-400" />
                    <span className="font-medium truncate">{usersMap[item.app.applicantId] || 'Tiada Rekod'}</span>
                  </div>
                </div>

                <div className="flex items-center justify-end pt-2 border-t border-slate-50">
                  <button className="text-blue-600 text-xs font-bold px-3 py-1.5 bg-blue-50 rounded-lg">Semak Laporan</button>
                </div>
              </div>
            ))}
            {displayedReports.length === 0 && (
              <div className="p-12 text-center text-slate-500 text-sm">Tiada laporan untuk disemak.</div>
            )}
          </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {displayedReports.map((item) => (
                  <tr key={item.app.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="p-3 sm:p-4 text-xs sm:text-sm font-medium text-slate-600">{item.report?.id || '-'}</td>
                    <td className="p-3 sm:p-4 text-xs sm:text-sm font-semibold text-slate-900 min-w-[150px]">{item.app.title}</td>
                    <td className="p-3 sm:p-4 text-xs sm:text-sm text-slate-600 whitespace-nowrap">{usersMap[item.app.applicantId] || 'Tiada Rekod'}</td>
                    <td className="p-3 sm:p-4 text-xs sm:text-sm text-slate-600 whitespace-nowrap">
                      {new Date(item.app.startDate || (item.app as any).date).toLocaleDateString('ms-MY', { day: 'numeric', month: 'short', year: 'numeric' })}
                    </td>
                    <td className="p-3 sm:p-4">
                      <span className={`inline-flex items-center px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-semibold border ${
                        item.status === 'Tertunggak' ? 'bg-red-50 text-red-700 border-red-200' : 
                        item.status === 'Dihantar' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                        'bg-emerald-50 text-emerald-700 border-emerald-200'
                      }`}>
                ...
                {displayedReports.length === 0 && (
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-slate-500">Tiada laporan untuk disemak.</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      {(isStudent || selectedAppId) && (
        <div className="flex flex-col lg:grid lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          {isStudent && (
            <div className={`lg:col-span-1 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-auto lg:h-[600px] ${selectedAppId ? 'hidden lg:flex' : 'flex'}`}>
              ...
              <div className="flex-1 overflow-y-auto p-2 sm:p-3 space-y-2 max-h-[300px] lg:max-h-none">
                {displayedReports.map((item) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-1 sm:mb-2">
                      <span className="text-[9px] sm:text-[10px] font-bold text-slate-500">{item.app.id}</span>
                      <span className={`text-[9px] sm:text-[10px] px-1.5 py-0.5 rounded-full font-semibold border ${
                        item.status === 'Tertunggak' ? 'bg-red-50 text-red-700 border-red-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'
                      }`}>
                        {item.status}
                      </span>
                    </div>
                    <h4 className={`font-semibold text-[11px] sm:text-sm leading-tight ${selectedAppId === item.app.id ? 'text-blue-900' : 'text-slate-900'}`}>{item.app.title}</h4>
                ...
                {displayedReports.length === 0 && (
                  <div className="p-4 text-center text-slate-500 text-xs sm:text-sm">Tiada program yang memerlukan laporan.</div>
                )}
              </div>
            </div>
          )}
          ...
          {/* Detail/Upload Form */}
          <div className={`${isStudent ? 'lg:col-span-2' : 'col-span-3'} bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-8 h-auto lg:h-[600px] overflow-y-auto ${!selectedAppId && isStudent ? 'hidden lg:block' : 'block'}`}>
            {selectedItem ? (
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-4">
                  <div>
                    ...
                    <h3 className="text-base sm:text-xl font-bold text-slate-900 font-display tracking-tight leading-tight">
                      {isStudent ? `Hantar Laporan: ${selectedItem.app.title}` : `Semakan Laporan: ${selectedItem.app.title}`}
                    </h3>
                    <p className="text-[10px] sm:text-sm text-slate-500 mt-1">
                      {isStudent ? 'Lengkapkan semua dokumen yang diperlukan untuk pengesahan.' : 'Sila semak dokumen yang dihantar oleh pelajar.'}
                    </p>
                  </div>
                </div>
                ...
                <>
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 sm:p-5 mb-4 sm:mb-6 flex gap-3 sm:gap-4 items-start">
                    ...
                    <p className="text-[10px] sm:text-sm text-amber-800 leading-relaxed font-medium">
                      Sila pastikan semua resit telah disahkan oleh penasihat program sebelum dimuat naik. Anda tidak boleh memohon program baharu selagi laporan ini tidak diluluskan.
                    </p>
                  </div>

                    <form className="grid gap-4 sm:gap-5" onSubmit={(e) => { e.preventDefault(); handleCreateReport(); }}>
                      {/* Jumlah Kewangan Kesatuan Mahasiswa */}
                      <div className="bg-slate-50 p-4 sm:p-5 rounded-2xl border border-slate-200">
                        <label className="block text-xs sm:text-sm font-bold text-slate-700 mb-2">Jumlah Kewangan Kesatuan Mahasiswa yang Digunakan (RM)</label>
                        <div className="relative">
                          <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-xs sm:text-sm">RM</span>
                          <input 
                            type="number" 
                            step="0.01"
                            required
                        ...
                        <p className="text-[10px] text-slate-500 mt-2 italic">Sila masukkan jumlah sebenar peruntukan Kesatuan Mahasiswa yang telah dibelanjakan.</p>
                      </div>

                      {/* Bilangan Peserta Program */}
                      <div className="bg-slate-50 p-4 sm:p-5 rounded-2xl border border-slate-200">
                        <label className="block text-xs sm:text-sm font-bold text-slate-700 mb-2">Bilangan Peserta Program</label>
                        <div className="relative">
                          <input 
                            type="number" 
                            required
                        ...
                        <p className="text-[10px] text-slate-500 mt-2 italic">Sila masukkan jumlah peserta yang menyertai program ini.</p>
                      </div>
                    ...
                    >
                      ...
                      />
                      <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center shrink-0 transition-colors ${reportFile ? 'bg-emerald-100' : 'bg-blue-50 group-hover:bg-blue-100'}`}>
                        {reportFile ? <CheckCircle className="w-5 h-5 sm:w-6 sm:h-6 text-emerald-600" /> : <FileText className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] sm:text-xs text-slate-500 mt-0.5">
                          {reportFile ? `${(reportFile.size / 1024 / 1024).toFixed(2)} MB` : 'Format rasmi HEPA (Maksimum 5MB)'}
                        </p>
                      </div>
                      <button type="button" className="hidden sm:block px-5 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-700 text-sm font-semibold rounded-xl border border-slate-200 transition-colors">
                    ...
                    >
                      ...
                      />
                      <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center shrink-0 transition-colors ${receiptFile ? 'bg-emerald-100' : 'bg-amber-50 group-hover:bg-amber-100'}`}>
                        {receiptFile ? <CheckCircle className="w-5 h-5 sm:w-6 sm:h-6 text-emerald-600" /> : <DollarSign className="w-5 h-5 sm:w-6 sm:h-6 text-amber-600" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-slate-900 text-xs sm:text-sm truncate">
                        ...
                        <p className="text-[10px] sm:text-xs text-slate-500 mt-0.5">
                          {receiptFile ? `${(receiptFile.size / 1024 / 1024).toFixed(2)} MB` : 'Disatukan dalam 1 fail PDF (Maksimum 5MB)'}
                        </p>
                      </div>
                      <button type="button" className="hidden sm:block px-5 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-700 text-sm font-semibold rounded-xl border border-slate-200 transition-colors">
                ...
                <div className="space-y-6 sm:space-y-8">
                  <div className="grid gap-4 sm:gap-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div className="bg-blue-50 p-3 sm:p-5 rounded-2xl border border-blue-100 flex items-center justify-between gap-2">
                        <div className="min-w-0">
                          <h4 className="text-[10px] sm:text-sm font-bold text-blue-900 truncate">Jumlah Kewangan Digunakan</h4>
                          <p className="text-[9px] sm:text-xs text-blue-600 mt-0.5 truncate">Berdasarkan penyata kewangan</p>
                        </div>
                        <div className="text-sm sm:text-xl font-bold text-blue-700 whitespace-nowrap">
                          RM {selectedItem.report?.unionBudgetUsed?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '0.00'}
                        </div>
                      </div>

                      <div className="bg-emerald-50 p-3 sm:p-5 rounded-2xl border border-emerald-100 flex items-center justify-between gap-2">
                        <div className="min-w-0">
                          <h4 className="text-[10px] sm:text-sm font-bold text-emerald-900 truncate">Bilangan Peserta</h4>
                          <p className="text-[9px] sm:text-xs text-emerald-600 mt-0.5 truncate">Jumlah peserta program</p>
                        </div>
                    ...
                    <div className="border border-slate-200 rounded-2xl p-3 sm:p-5 flex items-center gap-3 sm:gap-5 bg-slate-50">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-slate-900 text-[11px] sm:text-sm truncate">Laporan Akhir Program.pdf</h4>
                        <p className="text-[9px] sm:text-[10px] text-slate-500 mt-0.5">Dimuat naik pada {selectedItem.report?.submittedAt ? new Date(selectedItem.report.submittedAt).toLocaleDateString() : '-'}</p>
                      </div>
                      <a 
                    ...
                    <div className="border border-slate-200 rounded-2xl p-3 sm:p-5 flex items-center gap-3 sm:gap-5 bg-slate-50">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-slate-900 text-[11px] sm:text-sm truncate">Resit_Kewangan.pdf</h4>
                        <p className="text-[9px] sm:text-[10px] text-slate-500 mt-0.5">Dimuat naik pada {selectedItem.report?.submittedAt ? new Date(selectedItem.report.submittedAt).toLocaleDateString() : '-'}</p>
                      </div>
                      <a 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 pt-4 sm:pt-6 border-t border-slate-100">
                    <div>
                      <label className="block text-xs sm:text-sm font-bold text-slate-700 mb-2">Pengesahan Bajet Digunakan (RM)</label>
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-xs sm:text-sm">RM</span>
                        <input 
                          type="number" 
                      ...
                      <p className="text-[10px] text-slate-500 mt-2 italic">Sila sahkan jumlah peruntukan yang telah disemak.</p>
                    </div>

                    <div>
                      <label className="block text-xs sm:text-sm font-bold text-slate-700 mb-2">Ulasan Semakan</label>
                      <textarea 
                  ...
                  <div className="flex flex-col sm:flex-row justify-end gap-3 mt-4">
                    ...
                    >
                      <XCircle className="w-4 h-4" /> {submitting ? 'Memproses...' : 'Tolak / Minta Pembetulan'}
                    </button>
                    <button 
                    ...
                    >
                      <CheckCircle className="w-4 h-4" /> {submitting ? 'Memproses...' : 'Sahkan Laporan'}
                    </button>
                  </div>
                </div>
              )}
